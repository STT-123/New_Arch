...
// CAN 状态检查函数
int is_can_interface_ready(const char* can_if) {
    char command[128];
    snprintf(command, sizeof(command), 
             "/bin/ip link show %s 2>/dev/null | grep -q 'LOWER_UP'", can_if);
    
    int ret = system(command);
    return (ret == 0);  // 返回0表示CAN接口正常
}

// 带重试的CAN状态检查
int wait_for_can_ready(const char* can_if, int max_retries, int retry_interval_ms) {
    for (int i = 0; i < max_retries; i++) {
        if (is_can_interface_ready(can_if)) {
            printf("CAN interface %s is ready\n", can_if);
            return 1;  // 成功
        }
        
        printf("Waiting for CAN interface %s to be ready... (%d/%d)\n", 
               can_if, i+1, max_retries);
        usleep(retry_interval_ms * 1000);
    }
    
    printf("CAN interface %s not ready after %d retries\n", can_if, max_retries);
    return 0;  // 失败
}
/**
 * 检测CAN 是否异常函数
